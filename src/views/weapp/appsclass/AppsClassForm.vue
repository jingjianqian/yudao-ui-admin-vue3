...
/** 小程序分类 表单 */
defineOptions({ name: 'AppsClassForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await AppsClassApi.getAppsClass(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as AppsClassVO
    if (formType.value === 'create') {
      await AppsClassApi.createAppsClass(data)
      message.success(t('common.createSuccess'))
    } else {
      await AppsClassApi.updateAppsClass(data)
